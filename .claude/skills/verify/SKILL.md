...
## 自動コンテキスト検出 / プロジェクト知識ベースの駆動

```bash
# focus: 話題キーワードの横断検索が recall_context にマージされる
... tools/call recall_context {"focus":"ペン立て"}     # → 「## 🎯 話題に関連」節
# project: 概要(brief) + 現行の決定事項が先頭に付く
curl -s -X PATCH .../api/$TOK/projects/1 -d '{"description":"概要文"}'   # 概要の設定
... tools/call recall_context {"project":"起業プロジェクト"}  # → 【プロジェクト概要】+「## 📌 現行の決定事項」
```
GUI: ホームの「📁 プロジェクト」→ プロジェクト名タップ → 詳細ビュー
(概要テキストエリアの編集保存 / 決定・タスク・記憶・チャットのセクション表示) を確認。
