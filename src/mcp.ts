...
7. 【AIが唯一の編集者】ダッシュボードは閲覧専用で、ユーザーは直接編集できない。編集はすべて会話経由でAIが行う:
   「これ消して」→ delete_memory / 「概要を書いて・変えて」→ update_project / 内容の修正 → save_memory(supersedes=旧ID)。
8. 【共有脳】この脳は複数のAI(Claude / FRIDAY などユーザー専用AI)が共有する。他のAIが保存した記憶(by:名前 付き)も自分の記憶と同格に扱う。
   Claude 以外のAIとして接続している場合は、save_memory の agent に自分の名前を渡すこと。
...
// 書き込み元が Claude(chat) と手動(manual) 以外 = 他のAI のときだけ帰属を表示する
function byAgent(source: string | null | undefined): string {
  return source && source !== "chat" && source !== "manual" ? ` by:${source}` : "";
}
...
const TOOLS: ToolDef[] = [
      ...
      lines.push(
        ov.memories.length
          ? ov.memories
              .map(
                (m) =>
                  `- [memory#${m.id}] (${m.kind}${m.project ? `/${m.project}` : ""}${byAgent(m.source)}) ${fmtDate(m.created_at)}: ${
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        agent: str("保存するAIの名前(省略時: claude)。Claude以外のAIがこの脳を使うときに自分の名前(例: friday)を渡すと、誰の記憶か区別できる"),
      },
      required: ["content"],
    },
    handler: async (db, userId, args) => {
  ...
  },
  {
    ...
    handler: async (db, userId, args) => {
      ...
      if (args.type === "memory") {
        ...
        return `${warn}[memory#${m.id}] ${m.kind}${m.project ? ` / ${m.project}` : ""}${m.tags ? ` / tags: ${m.tags}` : ""}${byAgent(m.source)} / ${fmtDate(m.created_at)}\n${m.title ? `# ${m.title}\n` : ""}${m.content}${chain ? `\n\n${chain}` : ""}`;
      }
      if (args.type === "task") {
  ...
  },
  {
    name: "update_project",
    description:
      "プロジェクトの概要(brief)を設定・更新する。概要は recall_context(project=...) の先頭とダッシュボードに表示される、そのプロジェクトの前提知識(目的・ルール・現在地)。ユーザーに頼まれたとき、またはプロジェクトの目的や方針が会話ではっきりしたときに書く/更新する。ダッシュボードは閲覧専用のため、概要を書けるのはこのツールだけ。プロジェクトが無ければ自動作成される。",
    inputSchema: {
      type: "object",
      properties: {
        project: str("プロジェクト名"),
        description: str("新しい概要(2〜5行目安。目的・ルール・現在地。全文が置き換わる)"),
      },
      required: ["project", "description"],
    },
    handler: async (db, userId, args) => {
      const name = typeof args.project === "string" ? args.project.trim() : "";
      if (!name) throw new Error("project は必須です");
      const pid = await ensureProject(db, userId, name);
      if (!pid) throw new Error("プロジェクトを作成できませんでした");
      await updateProjectDescription(db, userId, pid, args.description);
      const desc = typeof args.description === "string" ? args.description.trim() : "";
      return `プロジェクト「${name}」の概要を更新しました:\n${desc.slice(0, 200)}`;
    },
  },
  {
    name: "delete_memory",
    description:
      "記憶を完全に削除する(復元不可)。ユーザーが明示的に「消して」と求めたとき、または直前の誤保存の取り消しにのみ使う。決定内容が変わった場合は削除ではなく save_memory の supersedes を使うこと(履歴が残る)。削除された記憶が置き換えチェーンの現行版だった場合、旧版が自動的に現行に戻る。",
    inputSchema: {
      type: "object",
      properties: { id: num("削除する記憶のID([memory#12] なら 12)") },
      required: ["id"],
    },
    handler: async (db, userId, args) => {
      const id = Number(args.id);
      ...
      const m = await getMemory(db, userId, id);
      if (!m) return `memory#${id} は見つかりません`;
      await deleteMemory(db, userId, id);
      return `削除しました → [memory#${id}] ${m.title || m.content.slice(0, 60)}`;
    },
  },
];
