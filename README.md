...
## 🧭 AI意思決定OS（アプリとしてインストール可）

同じ Worker に「AI意思決定OS」(メンター・監視官・記録官・作業AI の意思決定支援アプリ) が同居しています。

- URL: `https://<あなたのWorker>/os/<トークン>`（ダッシュボード右上の **🧭 意思決定OS** からも開けます）
- **PCにアプリとして入れる**: Chrome / Edge でOSを開く → アドレスバー右の **インストールアイコン**（または左メニュー下の「⬇ アプリとしてインストール」）→ 独立ウィンドウ・タスクバーから起動できるようになります
- **スマホ**: iPhone は共有 → 「ホーム画面に追加」/ Android は Chrome のメニュー → 「アプリをインストール」
- 各役割を実際に思考させるには Cloudflare の Secret に APIキー（`GEMINI_API_KEY` / `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` のどれか）を設定し、OS内の ⚙️ で役割ごとのモデルを選びます（未設定でもスタブ応答で全機能を試せます）
